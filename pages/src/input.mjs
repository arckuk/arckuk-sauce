...
export async function main() {
	...
	common.subscribe('athlete/watching', async watching => {
		...
		if ((worldtime_new - worldtime_old) >1000) {
			...
			if (common.settingsStore.get('showTenthsdegree') == true) {
				gradient = (watching.state.grade*100).toFixed(1);
				if (gradient == "-0.0") {
					gradient = "0.0"
				}
			} else {
				gradient = (watching.state.grade*100).toFixed(0);
				if ((gradient == "-0")) {
					gradient = "0"
				}
			}
			document.getElementById('grade').innerHTML = gradient;
	});
}
